...
export default function WebinarPage() {
  const { data: webinars } = useListWebinars({
    query: { queryKey: getListWebinarsQueryKey() }
  });

  const liveWebinar = webinars?.find(w => w.status === 'live');
  const upcomingWebinars = webinars?.filter(w => w.status === 'upcoming').sort((a, b) => new Date(a.scheduledAt).getTime() - new Date(b.scheduledAt).getTime()) || [];
  ...
  return (
    <div className="flex h-full w-full bg-background">
      {/* Left Panel - Chat/Alerts Placeholder */}
      <div className="w-72 bg-sidebar border-r border-border flex flex-col shrink-0">
            ...
            <div className="space-y-3">
              {upcomingWebinars.slice(0, 3).map(w => (
                <div key={w.id} className="bg-card border border-border p-3 rounded-lg">
                  <div className="text-xs text-primary font-mono mb-1">{format(new Date(w.scheduledAt), "MMM d, h:mm a")}</div>
            ...
            <div className="space-y-6">
              ...
              <div className="aspect-video bg-black rounded-xl border border-border flex items-center justify-center relative overflow-hidden group">
                {mainWebinar.thumbnailUrl && (
                  <img src={mainWebinar.thumbnailUrl} alt={mainWebinar.title} className="absolute inset-0 w-full h-full object-cover opacity-50 group-hover:opacity-40 transition-opacity" />
                )}
                <div className="relative z-10 flex flex-col items-center">
                  <div className="w-20 h-20 bg-primary/90 text-primary-foreground rounded-full flex items-center justify-center shadow-lg cursor-pointer hover:scale-105 transition-transform">
                    <Play className="w-8 h-8 ml-1" fill="currentColor" />
                  </div>
                  {mainWebinar.status === 'upcoming' && (
                    <div className="mt-6 bg-background/80 backdrop-blur-sm border border-border px-4 py-2 rounded-full font-mono text-sm">
                ...
                {mainWebinar.status === 'live' && (
                  <div className="absolute top-4 left-4 bg-destructive text-destructive-foreground px-3 py-1 rounded-full text-xs font-bold flex items-center gap-2 uppercase tracking-wider animate-pulse">
              ...
              <div className="space-y-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h1 className="text-3xl font-bold tracking-tight mb-2">{mainWebinar.title}</h1>
                    <div className="flex items-center gap-4 text-muted-foreground text-sm">
                      <span className="flex items-center gap-1.5"><CalendarIcon className="w-4 h-4" /> {format(new Date(mainWebinar.scheduledAt), "MMMM d, yyyy")}</span>
                      <span className="flex items-center gap-1.5"><Clock className="w-4 h-4" /> {format(new Date(mainWebinar.scheduledAt), "h:mm a")}</span>
                      <span>Hosted by <strong className="text-foreground">{mainWebinar.hostName}</strong></span>
                    </div>
                  </div>
                  <Button size="lg" className="shrink-0 font-bold">
                ...
                {pastWebinars.map(w => (
                  <div key={w.id} className="group cursor-pointer">
                    <div className="aspect-video bg-muted rounded-lg border border-border mb-3 relative overflow-hidden">
                      {w.thumbnailUrl && <img src={w.thumbnailUrl} className="absolute inset-0 w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" alt="" />}
